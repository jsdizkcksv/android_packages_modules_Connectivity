...
#undef REQUIRE
    ...
    if (!createDir("/sys/fs/bpf/tethering")) {
	    failed = true;
    }
    // This is technically T+ but S also needs it for the 'mainline_done' file.
    if (!createDir("/sys/fs/bpf/netd_shared")) {
	    failed = true;
    }

    if (isAtLeastT) {
        if (!createDir("/sys/fs/bpf/netd_readonly")) {
		failed = true;
	}
        if (!createDir("/sys/fs/bpf/net_shared")) {
		failed = true;
	}
        if (!createDir("/sys/fs/bpf/net_private")) {
		failed = true;
	}

        // This one is primarily meant for triggering genfscon rules.
        if (!createDir("/sys/fs/bpf/loader")) {
		failed = true;
	}
    }
